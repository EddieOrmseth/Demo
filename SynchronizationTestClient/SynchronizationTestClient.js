...
function findWasmBinary() {
  return locateFile("SynchronizationTestClient.wasm");
}
...
function _emglfw3c_init(context, scale, scaleChangeCallback, windowResizeCallback, keyboardCallback, clipboardCallback, requestFullscreen, errorHandler) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(11, 0, 1, context, scale, scaleChangeCallback, windowResizeCallback, keyboardCallback, clipboardCallback, requestFullscreen, errorHandler);
  context >>>= 0;
...
function _emscripten_glfw3_destroy_custom_cursor(glfwCursor) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(29, 0, 1, glfwCursor);
  glfwCursor >>>= 0;
...
function _emscripten_html5_remove_event_listener(target, userData, eventTypeId, callback) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(30, 0, 1, target, userData, eventTypeId, callback);
  target >>>= 0;
  userData >>>= 0;
  callback >>>= 0;
  var eventHandler = {
...
function _emscripten_request_pointerlock(target, deferUntilInEventHandler) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(34, 0, 1, target, deferUntilInEventHandler);
  target >>>= 0;
  target = findEventTarget(target);
  if (!target) return -4;
  if (!target.requestPointerLock) {
...
function _emscripten_stack_unwind_buffer(addr, buffer, count) {
  ...
  for (var i = 0; i < count && stack[i + offset]; ++i) {
    (growMemViews(), HEAP32)[(((buffer) + (i * 4)) >>> 2) >>> 0] = convertFrameToPC(stack[i + offset]);
  }
  return i;
}
