...
function findWasmBinary() {
  return locateFile("Development.wasm");
}
...
function _emglfw3w_change_focus(glfwWindow, isFocussed) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(9, 0, 1, glfwWindow, isFocussed);
  glfwWindow >>>= 0;
...
function _emglfw3w_destroy(glfwWindow) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(10, 0, 1, glfwWindow);
  glfwWindow >>>= 0;
...
function _emglfw3w_set_size(glfwWindow, width, height, fbWidth, fbHeight) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(11, 0, 1, glfwWindow, width, height, fbWidth, fbHeight);
  glfwWindow >>>= 0;
  const ctx = GLFW3.fWindowContexts[glfwWindow];
  const canvas = ctx.canvas;
...
function _emglfw3w_set_standard_cursor(glfwWindow, cursor) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(12, 0, 1, glfwWindow, cursor);
  glfwWindow >>>= 0;
  cursor >>>= 0;
  const ctx = GLFW3.fWindowContexts[glfwWindow];
...
function _emscripten_html5_remove_event_listener(target, userData, eventTypeId, callback) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(13, 0, 1, target, userData, eventTypeId, callback);
  target >>>= 0;
  userData >>>= 0;
  callback >>>= 0;
  var eventHandler = {
...
var proxiedFunctionTable = [ _proc_exit, exitOnMainThread, pthreadCreateProxied, ___syscall_fcntl64, ___syscall_ioctl, ___syscall_openat, _emglfw3c_destroy, _emglfw3c_get_fullscreen_window, _emglfw3c_get_pointer_lock_window, _emglfw3w_change_focus, _emglfw3w_destroy, _emglfw3w_set_size, _emglfw3w_set_standard_cursor, _emscripten_html5_remove_event_listener, _emscripten_set_fullscreenchange_callback_on_thread, _emscripten_set_gamepadconnected_callback_on_thread, _emscripten_sample_gamepad_data, _emscripten_set_gamepaddisconnected_callback_on_thread, _emscripten_set_mousemove_callback_on_thread, _emscripten_set_mouseup_callback_on_thread, _emscripten_set_pointerlockchange_callback_on_thread, _emscripten_set_pointerlockerror_callback_on_thread, _emscripten_set_touchcancel_callback_on_thread, _emscripten_set_touchend_callback_on_thread, _emscripten_set_touchmove_callback_on_thread, _emscripten_set_touchstart_callback_on_thread, _environ_get, _environ_sizes_get, _fd_close, _fd_read, _fd_seek, _fd_write ];
