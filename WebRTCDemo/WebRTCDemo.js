...
function initMemory() {
  ...
  if (Module["wasmMemory"]) {
    ...
    assert(INITIAL_MEMORY >= 1073741824, `INITIAL_MEMORY should be larger than STACK_SIZE, was ${INITIAL_MEMORY}! (STACK_SIZE=1073741824)`);
...
function findWasmBinary() {
  return locateFile("WebRTCDemo.wasm");
}
...
function __emscripten_init_main_thread_js(tb) {
  ...
  __emscripten_thread_init(tb, /*is_main=*/ !ENVIRONMENT_IS_WORKER, /*is_runtime=*/ 1, /*can_block=*/ !ENVIRONMENT_IS_WEB, /*default_stacksize=*/ 1073741824, /*start_profiling=*/ false);
  PThread.threadInitTLS();
}
...
var GLFW3 = {
  ...
  onKeyboardEvent(e) {
    ...
    if (GLFW3.fKeyboardCallback) {
      ...
      if (((a1, a2, a3, a4, a5, a6, a7) => dynCall_iiiiiiii(GLFW3.fKeyboardCallback, a1, a2, a3, a4, a5, a6, a7))(GLFW3.fContext, e.type === "keydown", code, key, e.repeat, codepoint, modifierBits)) e.preventDefault();
      _free(key);
      _free(code);
    }
  },
...
function _emglfw3c_init(context, scale, scaleChangeCallback, windowResizeCallback, keyboardCallback, clipboardCallback, requestFullscreen, errorHandler) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(9, 0, 1, context, scale, scaleChangeCallback, windowResizeCallback, keyboardCallback, clipboardCallback, requestFullscreen, errorHandler);
  context >>>= 0;
  scaleChangeCallback >>>= 0;
...
var _emglfw3c_set_clipboard_string = function(content) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(12, 0, 1, content);
  content >>>= 0;
...
function _emglfw3c_set_title(title) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(13, 0, 1, title);
  title >>>= 0;
  if (title) document.title = UTF8ToString(title);
}

function _emglfw3w_change_focus(glfwWindow, isFocussed) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(14, 0, 1, glfwWindow, isFocussed);
  glfwWindow >>>= 0;
...
function _emglfw3w_destroy(glfwWindow) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(15, 0, 1, glfwWindow);
  glfwWindow >>>= 0;
...
function _emglfw3w_get_computed_opacity(glfwWindow) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(16, 0, 1, glfwWindow);
  glfwWindow >>>= 0;
...
function _emglfw3w_get_position(glfwWindow, x, y) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(18, 0, 1, glfwWindow, x, y);
  glfwWindow >>>= 0;
  x >>>= 0;
  y >>>= 0;
  const canvas = GLFW3.fWindowContexts[glfwWindow].canvas;
  const rect = getBoundingClientRect(canvas);
  (growMemViews(), HEAP32)[((x) >>> 2) >>> 0] = rect.x;
  (growMemViews(), HEAP32)[((y) >>> 2) >>> 0] = rect.y;
}
...
function _emglfw3w_init(glfwWindow, canvasSelector) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(19, 0, 1, glfwWindow, canvasSelector);
  glfwWindow >>>= 0;
  canvasSelector >>>= 0;
...
function _emglfw3w_on_created(glfwWindow) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(20, 0, 1, glfwWindow);
  glfwWindow >>>= 0;
...
function _emglfw3w_set_size(glfwWindow, width, height, fbWidth, fbHeight) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(21, 0, 1, glfwWindow, width, height, fbWidth, fbHeight);
  glfwWindow >>>= 0;
  const ctx = GLFW3.fWindowContexts[glfwWindow];
  const canvas = ctx.canvas;
...
function _emglfw3w_set_standard_cursor(glfwWindow, cursor) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(22, 0, 1, glfwWindow, cursor);
  glfwWindow >>>= 0;
  cursor >>>= 0;
  const ctx = GLFW3.fWindowContexts[glfwWindow];
...
function _emglfw3w_set_visibility(glfwWindow, visible) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(23, 0, 1, glfwWindow, visible);
  glfwWindow >>>= 0;
  const ctx = GLFW3.fWindowContexts[glfwWindow];
...
var _emscripten_glMultiDrawArraysInstancedBaseInstanceWEBGL = (mode, firsts, counts, instanceCounts, baseInstances, drawCount) => {
  GLctx.mdibvbi["multiDrawArraysInstancedBaseInstanceWEBGL"](mode, (growMemViews(), 
  HEAP32), ((firsts) >>> 2), (growMemViews(), HEAP32), ((counts) >>> 2), (growMemViews(), 
  HEAP32), ((instanceCounts) >>> 2), (growMemViews(), HEAPU32), ((baseInstances) >>> 2), drawCount);
};
...
var _emscripten_glMultiDrawElementsInstancedBaseVertexBaseInstanceWEBGL = (mode, counts, type, offsets, instanceCounts, baseVertices, baseInstances, drawCount) => {
  GLctx.mdibvbi["multiDrawElementsInstancedBaseVertexBaseInstanceWEBGL"](mode, (growMemViews(), 
  HEAP32), ((counts) >>> 2), type, (growMemViews(), HEAP32), ((offsets) >>> 2), (growMemViews(), 
  HEAP32), ((instanceCounts) >>> 2), (growMemViews(), HEAP32), ((baseVertices) >>> 2), (growMemViews(), 
  HEAPU32), ((baseInstances) >>> 2), drawCount);
};
...
function _emscripten_glfw3_destroy_custom_cursor(glfwCursor) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(27, 0, 1, glfwCursor);
  glfwCursor >>>= 0;
...
function _emscripten_html5_remove_event_listener(target, userData, eventTypeId, callback) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(28, 0, 1, target, userData, eventTypeId, callback);
  target >>>= 0;
  userData >>>= 0;
  callback >>>= 0;
  var eventHandler = {
...
var _wsSetErrorCallback = function(ws, errorCallback) {
  ...
  var cb = function() {
    ...
    ((a1, a2) => dynCall_vii(errorCallback, a1, a2))(0, userPointer);
  };
...
function assignWasmExports(wasmExports) {
  ...
  assert(typeof wasmExports["dynCall_idd"] != "undefined", "missing Wasm export: dynCall_idd");
  ...
  dynCall_viiffff = dynCalls["viiffff"] = createExportWrapper("dynCall_viiffff", 7);
  ...
  dynCall_idd = dynCalls["idd"] = createExportWrapper("dynCall_idd", 3);
